defmodule NVim.ApiSpec do
  ...
  {:ok, [_channel, %{"functions" => function_specs}] = _api_info} =
    RPC.Session.call(@session_name, "vim_get_api_info", [])
  ...
  it "injects and proxies vim API" do
    expect(@session_name.vim_get_api_info()).to eq(
      RPC.Session.call(@session_name, "vim_get_api_info", [])
    )
  end

  it "injects with params" do
    expect(@session_name.vim_command_output("echo 123")).to eq({:ok, "\n123"})
  end

  it "works with types" do
    import @session_name

    {:ok, buffer} = vim_get_current_buffer
    expect(buffer_set_line(buffer, 0, "test")).to eq({:ok, nil})
    expect(vim_get_current_line).to eq({:ok, "test"})
  end
end
